...
function collectSlugCandidates(product) {
...
function collectSubcategoryCandidates(product) {
    const specs = product?.specs || {};
...
export function getProductRouteAliases(product) {
    const canonicalPath = getProductCanonicalPath(product);
    const canonicalSlug = normalizeValue(canonicalPath.split("/").pop() || "");
    const specs = product?.specs || {};
    const parentSlug = normalizeValue(specs?.parentCategorySlug || specs?.categorySlug || product?.category || "electronics");
...
export function findProductRouteMatch(products, identifier, categorySlug, subcategorySlug) {
