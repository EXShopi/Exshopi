...
function findNodeBySlug(list, slug) {
    const target = normalizeCategorySlug(slug);
    for (const node of list || []) {
        if (normalizeCategorySlug(node.slug) === target)
            return { node, parent: null };
        if (Array.isArray(node.subcategories)) {
            for (const sub of node.subcategories) {
                if (normalizeCategorySlug(sub.slug) === target)
                    return { node: sub, parent: node };
                if (Array.isArray(sub.childCategories)) {
                    for (const child of sub.childCategories) {
                        if (normalizeCategorySlug(child.slug) === target)
                            return { node: child, parent: sub };
                    }
                }
            }
        }
...
export function getChildCategories(parentSlug, subSlug) {
    const parent = MASTER_CATEGORIES.find((p) => normalizeCategorySlug(p.slug) === normalizeCategorySlug(parentSlug));
    if (!parent)
        return [];
...
export function getCategoryRouteInfo(categorySlug, subcategorySlug) {
    ...
    if (normalizedSubcategory) {
        ...
        const canonicalCategorySlug = normalizeCategorySlug(parentLookup?.parentCategorySlug ||
            parentLookup?.categorySlug ||
            childLookup?.parentCategorySlug ||
            normalizedCategory);
        const canonicalSubcategorySlug = normalizeCategorySlug(childLookup?.depth === 1
            ? childLookup.parentCategorySlug
    ...
    const resolved = findCanonicalPathBySlug(normalizedCategory) ||
...
function buildAssignmentFromPath(parent, category, subcategory, matchedBy = []) {
    if (!parent && !category && !subcategory)
        return null;
    ...
    return {
        ...
        categorySlug: normalizeCategorySlug((resolvedCategory && resolvedCategory !== resolvedParent ? resolvedCategory.slug : resolvedParent?.slug) || ""),
        subcategorySlug: normalizeCategorySlug(resolvedSubcategory?.slug || ""),
        parentCategoryName: String(resolvedParent?.name || ""),
        categoryName: String((resolvedCategory && resolvedCategory !== resolvedParent ? resolvedCategory.name : resolvedParent?.name) || ""),
...
function findCanonicalPathBySlug(value) {
...
function findCanonicalPathByLegacyValue(value) {
...
export function debugCategoryAssignment(label, payload) {
    if (!shouldDebugCategoryLogs())
        return;
    try {
...
export function resolveCanonicalCategoryAssignment(input) {
    ...
    const subcategoryInput = source.subcategorySlug ||
    ...
    const parentCategorySlug = normalizeCategorySlug(explicitParent?.parentCategorySlug ||
        explicitParent?.categorySlug ||
        best?.parentCategorySlug ||
        best?.categorySlug ||
        categoryPathParts[0] ||
        "");
    const categorySlug = normalizeCategorySlug((best && best.depth >= 2 ? best.categorySlug : "") ||
        ...
        parentCategorySlug);
    const subcategorySlug = normalizeCategorySlug((best && best.depth >= 3 ? best.subcategorySlug : "") ||
        categoryPathParts[2] ||
        (best && best.depth >= 2
            ? ""
    ...
    const finalCategory = canonicalSubPath?.categoryName ||
...
export function productMatchesCategoryAssignment(product, categorySlug, subcategorySlug, parentSlug) {
...
export function mapLegacyCategory(value) {
    ...
    if (!v)
        return null;
    if (LEGACY_MAP[v])
        return LEGACY_MAP[v][0] || null;
    const tokens = v.split(/[-_\s]+/).filter(Boolean);
    for (const t of tokens) {
        if (LEGACY_MAP[t])
            return LEGACY_MAP[t][0] || null;
    }
...
export function gatherSlugsUnder(categorySlug) {
    const cacheKey = normalizeCategorySlug(categorySlug || '');
    if (!gatherSlugsUnder._cache) {
        gatherSlugsUnder._cache = new Map();
    }
    const cache = gatherSlugsUnder._cache;
    ...
    function walk(node) {
        ...
        if (Array.isArray(node.childCategories)) {
            for (const c of node.childCategories)
                walk(c);
        }
    }
    walk(found.node);
...
export function filterProductsByCategoryTree(products, categorySlug, subcategorySlug, backendCategories) {
    if (!categorySlug)
        return [];
    ...
    const result = (products || []).filter((product) => {
        ...
        const hasResolvedAssignment = Boolean(resolvedAssignment.parentCategorySlug || resolvedAssignment.categorySlug || resolvedAssignment.subcategorySlug);
        ...
        if (catId && backendCategories && Array.isArray(backendCategories)) {
            ...
            if (byId?.slug)
                values.push(normalizeCategorySlug(byId.slug));
        }
        if (product?.specs) {
            if (product.specs.parentCategorySlug)
                values.push(normalizeCategorySlug(product.specs.parentCategorySlug));
            if (product.specs.categorySlug)
                values.push(normalizeCategorySlug(product.specs.categorySlug));
            if (product.specs.subcategorySlug)
                values.push(normalizeCategorySlug(product.specs.subcategorySlug));
            if (product.specs.childCategorySlug)
                values.push(normalizeCategorySlug(product.specs.childCategorySlug));
            if (product.specs.templateId)
                values.push(normalizeCategorySlug(String(product.specs.templateId)));
            if (product.specs.parentCategoryName)
                values.push(normalizeCategorySlug(String(product.specs.parentCategoryName)));
            if (product.specs.categoryName)
                values.push(normalizeCategorySlug(String(product.specs.categoryName)));
            if (product.specs.categoryPath) {
                values.push(...String(product.specs.categoryPath || '')
                    .split('/')
                    .map((s) => normalizeCategorySlug(String(s))));
            }
            if (product.specs.parentCategory)
                values.push(normalizeCategorySlug(String(product.specs.parentCategory)));
        ...
        if (product?.category)
            values.push(normalizeCategorySlug(String(product.category)));
        if (product?.subcategory)
            values.push(normalizeCategorySlug(String(product.subcategory)));
        if (product?.brand)
            values.push(normalizeCategorySlug(String(product.brand)));
        if (product?.tags && Array.isArray(product.tags)) {
            for (const t of product.tags)
                values.push(normalizeCategorySlug(String(t)));
        }
        if (product?.labels && Array.isArray(product.labels)) {
            for (const l of product.labels)
                values.push(normalizeCategorySlug(String(l)));
        }
        ...
        const legacySource = product?.category ||
