...
const DEFAULT_SITE_URL = (typeof import.meta !== "undefined" && String(import.meta?.env?.VITE_SITE_URL || "").trim()) ||
    "https://exshopi.com";
const UAE_KEYWORDS = [
    "UAE",
    "Dubai",
...
export function trimSeoKeywords(value) {
    return uniqueKeywords(String(value || "")
        .split(",")
        .map((item) => normalizeSeoText(item))
        .filter(Boolean)).join(", ");
}
export function getSeoLengthStatus(value, minimum, maximum) {
...
export function generateAdminSeoFields(input) {
    ...
    const metaDescription = buildSeoDescription(`${normalizeSeoText(input.shortDescription) || title} Shop now on ExShopi with UAE delivery, trusted support, and marketplace-ready product details.`, `${title} available on ExShopi with UAE delivery, trusted support, and premium marketplace service.`);
    const metaKeywords = trimSeoKeywords([title, category, `${category} UAE`, "buy electronics UAE", "Dubai", "ExShopi"]
        .filter(Boolean)
        .join(", "));
...
export function getProductCategorySlugs(product) {
    ...
    const parentSlug = String(resolved.parentCategorySlug ||
        ...
        .trim()
        .toLowerCase();
    const subcategorySlug = String(resolved.subcategorySlug ||
...
export function getProductSlug(product) {
    return slugifySeo(product?.slug ||
        product?.seoSlug ||
...
export function getCategoryPath(categorySlug, subcategorySlug) {
...
export function generateProductMeta(product) {
    ...
    const storedTitle = product?.metaTitle || product?.specs?.metaTitle || product?.specs?.seo?.metaTitle || "";
    const storedDescription = product?.metaDescription ||
        product?.specs?.metaDescription ||
        product?.specs?.seo?.metaDescription ||
        "";
    const storedKeywords = product?.metaKeywords ||
        product?.specs?.metaKeywords ||
        product?.specs?.seo?.metaKeywords ||
        "";
    const generatedTitle = clampText(storedTitle ||
        `Buy ${buildRichProductTitle(product) || title} in UAE | Best Price | ${DEFAULT_SITE_NAME}`, 60);
    const generatedDescription = clampText(storedDescription ||
        `Shop ${title} in UAE. Cash on Delivery, Fast Delivery, Warranty Available. Order now on ${DEFAULT_SITE_NAME}.`, 160);
    const keywordList = uniqueKeywords([
        ...
        ...FOCUS_KEYWORDS.filter((keyword) => {
            ...
            return (keyword.includes("electronics") ||
...
export function buildProductImageAlt(product, imageIndex = 0) {
    const title = String(product?.title || product?.name || "Product").trim();
    const brand = String(product?.brand || product?.specs?.attributes?.brand || "").trim();
    const model = String(product?.specs?.attributes?.model ||
        product?.specs?.model ||
    ...
    const storage = String(product?.specs?.attributes?.storage ||
...
export function generateCategorySeo(categoryName, categorySlug) {
    ...
    const description = clampText(`${normalizedName} on ExShopi for UAE and GCC shoppers. Compare trusted marketplace listings, secure COD checkout, and premium electronics deals with fast Dubai delivery.`, 160);
...
export function buildCategorySeoDescription(categoryName) {
    const title = String(categoryName || "products").trim();
    return (`${title} on ExShopi are curated for UAE and GCC shoppers who want better value, clear specifications, and trusted marketplace protection before they buy. ` +
...
export function generateHomepageSeo() {
    return {
        ...
        metaDescription: clampText("Shop electronics, mobiles, refurbished laptops, accessories and more in the UAE on ExShopi. Trusted sellers, fast delivery, and cash on delivery.", 160),
...
export function buildProductSchema(product, pathname) {
    const seo = generateProductMeta(product);
    const stock = Number(product?.stock || 0);
    const conditionText = String(product?.specs?.attributes?.condition ||
        product?.condition ||
    ...
    const sellerName = String(product?.sellerName ||
        ...
        "ExShopi").trim();
    const itemCondition = /used|renewed|refurb/i.test(conditionText)
        ? "https://schema.org/UsedCondition"
        : "https://schema.org/NewCondition";
    const description = String(product?.specs?.shortDescription ||
        product?.description ||
...
export function buildProductBreadcrumbSchema(product, pathname) {
    const parentName = product?.specs?.parentCategoryName ||
        product?.specs?.categoryName ||
        product?.category ||
        "Products";
    const parentSlug = product?.specs?.parentCategorySlug ||
        product?.specs?.categorySlug ||
        product?.category ||
        "products";
    const subcategoryName = product?.specs?.subcategoryName ||
        product?.specs?.templateName ||
        product?.specs?.subcategorySlug ||
        "";
    const subcategorySlug = product?.specs?.subcategorySlug ||
...
export function getDefaultPageTitle(pathname) {
    const normalized = String(pathname || "/").toLowerCase();
    if (normalized === "/")
        return generateHomepageSeo().metaTitle;
    if (normalized.startsWith("/products"))
        return "All Products in UAE | Premium Marketplace | ExShopi";
    if (normalized.startsWith("/blog"))
        return "ExShopi Blog | UAE Buying Guides, Reviews & Shopping Tips";
    if (normalized.startsWith("/contact"))
        return "Contact ExShopi | UAE Marketplace Support";
    if (normalized.startsWith("/about"))
        return "About ExShopi | UAE Marketplace";
    return `${DEFAULT_SITE_NAME} | Premium UAE Marketplace`;
}
export function getDefaultPageDescription(pathname) {
    const normalized = String(pathname || "/").toLowerCase();
    if (normalized === "/")
        return generateHomepageSeo().metaDescription;
    if (normalized.startsWith("/products")) {
