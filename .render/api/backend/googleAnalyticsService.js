...
function readServiceAccountFile() {
    const configuredPath = String(process.env.GA4_SERVICE_ACCOUNT_FILE ||
...
function readGoogleAnalyticsConfig() {
    ...
    const propertyId = normalizePropertyId(process.env.GA4_PROPERTY_ID ||
        ...
        '');
    const clientEmail = sanitizeEnvString(process.env.GA4_CLIENT_EMAIL ||
    ...
    const privateKey = normalizePrivateKey(process.env.GA4_PRIVATE_KEY ||
        ...
        '');
    const projectId = sanitizeEnvString(process.env.GA4_PROJECT_ID ||
        process.env.GOOGLE_ANALYTICS_PROJECT_ID ||
        parsedJson?.project_id ||
        fileJson?.project_id ||
        '');
    const clientId = sanitizeEnvString(process.env.GA4_CLIENT_ID ||
...
function getSafeConfigSummary(config) {
    return {
...
function ensureConfigStatusLogged(config) {
    if (configStatusLogged)
        return;
...
async function getAccessToken(config) {
    ...
    if (!response.ok || typeof payload?.access_token !== 'string') {
        throw new Error(`Failed to authorize Google Analytics service account (${response.status}): ${payload?.error_description || payload?.error || 'Unknown error'}`);
    }
...
async function executeAnalyticsRequest(config, method, body, ttlMs) {
    ...
    const response = await fetch(`${ANALYTICS_DATA_BASE_URL}/properties/${config.propertyId}:${method}`, {
        method: 'POST',
        headers: {
...
function mapReportRows(response, dimensionKeys, metricKeys) {
    return (response.rows || []).map((row) => {
        const dimensions = Object.fromEntries(dimensionKeys.map((key, index) => [key, String(row.dimensionValues?.[index]?.value || '')]));
        const metrics = Object.fromEntries(metricKeys.map((key, index) => [key, parseMetricValue(row.metricValues?.[index]?.value)]));
        return {
            ...dimensions,
            ...metrics,
        };
...
async function tryReportCandidates(config, candidates, ttlMs = 120_000) {
    const errors = [];
...
function buildTopList(rows, valueKey, limit = 8) {
    return rows
        .slice()
...
function inferProductPage(pathname) {
...
function inferCategoryPage(pathname) {
    const path = String(pathname || '').trim();
    if (!path || path === '/')
        return false;
    const segments = path.split('?')[0].split('/').filter(Boolean);
...
async function getGoogleAnalyticsReports(config, dateRange) {
    const dateRanges = buildDateRanges(dateRange);
    const summary = await executeAnalyticsRequest(config, 'runReport', {
        dateRanges,
        metrics: [
            { name: 'activeUsers' },
            { name: 'totalUsers' },
    ...
    const trend = await executeAnalyticsRequest(config, 'runReport', {
        dateRanges,
    ...
    const geographyCountry = await executeAnalyticsRequest(config, 'runReport', {
        dateRanges,
    ...
    const geographyCity = await executeAnalyticsRequest(config, 'runReport', {
        dateRanges,
    ...
    const geographyRegion = await executeAnalyticsRequest(config, 'runReport', {
        dateRanges,
    ...
    const deviceCategory = await executeAnalyticsRequest(config, 'runReport', {
        dateRanges,
    ...
    const browser = await executeAnalyticsRequest(config, 'runReport', {
        dateRanges,
    ...
    const operatingSystem = await executeAnalyticsRequest(config, 'runReport', {
        dateRanges,
    ...
    try {
        ...
        demographics = {
            ...
            reason: ageRows.length || genderRows.length || interestRows.length
                ? undefined
    ...
    const realtimeSummary = await executeAnalyticsRequest(config, 'runRealtimeReport', {
        metrics: [{ name: 'activeUsers' }, { name: 'eventCount' }],
        returnPropertyQuota: true,
    }, 45_000);
    const realtimeCountries = await executeAnalyticsRequest(config, 'runRealtimeReport', {
        dimensions: [{ name: 'country' }],
        metrics: [{ name: 'activeUsers' }],
        orderBys: [{ metric: { metricName: 'activeUsers' }, desc: true }],
        limit: '10',
    }, 45_000);
    const realtimeCities = await executeAnalyticsRequest(config, 'runRealtimeReport', {
        dimensions: [{ name: 'country' }, { name: 'city' }],
        metrics: [{ name: 'activeUsers' }],
        orderBys: [{ metric: { metricName: 'activeUsers' }, desc: true }],
        limit: '12',
    }, 45_000);
    const realtimeRegions = await executeAnalyticsRequest(config, 'runRealtimeReport', {
        dimensions: [{ name: 'country' }, { name: 'region' }],
        metrics: [{ name: 'activeUsers' }],
        orderBys: [{ metric: { metricName: 'activeUsers' }, desc: true }],
        limit: '12',
    ...
    const pageRows = mapReportRows(pageContent.response, [pageContent.key], ['screenPageViews', 'activeUsers', 'averageSessionDuration']).map((row) => ({
    ...
    const landingRows = mapReportRows(landingPages.response, [landingPages.key], ['sessions', 'activeUsers', 'keyEvents']).map((row) => ({
    ...
    const dubaiUsers = uaeCities
        ...
        uaeRegions
            ...
            .reduce((sum, row) => sum + Number(row.activeUsers || 0), 0);
    const topProductPages = buildTopList(pageRows.filter((row) => inferProductPage(row.path)), 'screenPageViews', 10);
    const topCategoryPages = buildTopList(pageRows.filter((row) => inferCategoryPage(row.path)), 'screenPageViews', 10);
    return {
        summary: {
        ...
        acquisition: {
            sourceMedium: mapReportRows(acquisition.response, [acquisition.key], acquisition.key === 'sessionSourceMedium'
        ...
        content: {
            ...
            inferenceNote: 'Top product and category pages are inferred from real GA paths using ExShopi route structure; no synthetic labels are added.',
        },
        demographics,
        realtime: {
...
export async function getGoogleAnalyticsAdminSnapshot(dateRange) {
    ...
    if (!config.enabled) {
        return {
            ...
            configurationIssue: config.missing.length > 0
            ...
            content: {
                ...
                inferenceNote: 'Google Analytics is not configured yet, so no traffic data is available from the property.',
            },
            demographics: {
                available: false,
                ageBrackets: [],
    ...
    try {
        ...
        return {
            ...
            content: {
                ...
                pathDimension: null,
                landingDimension: null,
                inferenceNote: 'Google Analytics could not be reached for this request, so only internal store metrics will be available.',
            },
            demographics: {
                available: false,
                ageBrackets: [],
