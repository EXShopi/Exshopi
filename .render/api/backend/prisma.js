...
const safeParseConnectionTarget = (value) => {
    if (!value) {
        return {
            host: 'missing',
            port: '',
...
const getConnectionIssues = (label, value) => {
...
export const getPrismaEnvDiagnostics = () => {
    ...
    const usePrismaRuntime = process.env.USE_PRISMA_RUNTIME === 'true' ||
...
export const prisma = global.__exshopiPrisma ||
    new PrismaClient({
