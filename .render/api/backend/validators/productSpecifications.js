import { buildDetailedSpecificationGroups, buildFlatSpecifications, getMissingRequiredSpecificationLabels, getSpecificationTemplate, humanizeSpecificationValue, normalizeSpecificationValues, } from '../../shared/productSpecifications';
...
function normalizeAdditionalGroups(raw) {
    if (!Array.isArray(raw))
        return [];
    return raw
        .map((group, index) => {
...
function normalizeVariantDimensions(raw, template) {
...
function normalizeVariants(raw, variantAttributes) {
    if (!Array.isArray(raw))
        return [];
    ...
    return raw
        ...
        const next = {
            ...
            price: source.price === '' || source.price == null ? null : Number.isFinite(Number(source.price)) ? Number(source.price) : null,
            originalPrice: source.originalPrice === '' || source.originalPrice == null
                ? null
                : Number.isFinite(Number(source.originalPrice))
                    ? Number(source.originalPrice)
                    : null,
            stock: source.stock === '' || source.stock == null ? null : Number.isFinite(Number(source.stock)) ? Number(source.stock) : null,
            sku: String(source.sku || '').trim(),
            image: String(source.image || '').trim() || undefined,
        };
        ['color', 'size', 'storage', 'ram', 'processor', 'material', 'style'].forEach((key) => {
...
export function normalizeProductSpecifications(inputSpecs, context) {
    ...
    const template = getSpecificationTemplate(canonicalAssignment.parentCategorySlug || context.parentCategorySlug || context.categorySlug || '', canonicalAssignment.subcategorySlug || context.subcategorySlug || specs.templateId || '', canonicalAssignment.subcategoryName || context.subcategoryName || specs.templateName || '', canonicalAssignment.categorySlug || context.categorySlug || context.parentCategorySlug || '');
    const sourceValues = {
    ...
    return {
        ...
        specs: {
            ...
            attributes: {
                ...flatSpecifications,
                brand: String(flatSpecifications.brand ||
                    humanizeSpecificationValue(specificationValues.brand) ||
...
export function validateProductSpecificationsForTemplate(specs, template, options = {}) {
