...
import { addBlacklistEntry, calculateCodRisk, consumeCodOtpVerification, createCodOtpSession, getNotificationsForAudience, isBlacklisted, isValidUaePhone, listBlacklistEntries, normalizePhone, pushNotification, verifyCodOtpSession, } from './codOps';
import { ADMIN_PERMISSION_MATRIX, hasAdminPermission, authMiddleware, canAccessFinance, canAccessSupport, hashPassword, isAdminLike, isBackofficeRole, signAccessToken, signRefreshToken, upgradeLegacyPasswordIfNeeded, verifyPassword, verifyRefreshToken, tryAuthenticateRequest, } from './auth';
...
const connectionMode = prismaRuntime.enabled
    ...
    : supabaseRuntime.enabled
        ? 'supabase'
        : 'json-db';
const seoSlugify = (value) => String(value || '')
    .toLowerCase()
    .trim()
    .replace(/\bcopy\b/g, '')
    .replace(/-copy(?:-\d+)?$/g, '')
...
const applyCorsHeaders = (req, res) => {
    ...
    res.setHeader('Access-Control-Allow-Headers', requestedHeaders.length ? requestedHeaders.join(', ') : defaultAllowedHeaders.join(','));
...
const CORE_BACKOFFICE_LOGIN_SEEDS = new Map([
    [normalizeAuthEmail('ahsansajid295@gmail.com'), 'T7&fD!2q'],
    [normalizeAuthEmail('official@exshopi.com'), 'exshopi-official'],
]);
const isCoreBackofficeCredential = (email, password) => CORE_BACKOFFICE_LOGIN_SEEDS.get(normalizeAuthEmail(email)) === String(password || '');
...
const getClientIp = (req) => String(req.headers['x-forwarded-for'] ||
...
const normalizeOperationalStatus = (status) => {
    ...
    if (value === 'placed')
        return 'pending_confirmation';
    if (value === 'ready_for_pickup' || value === 'pickup_scheduled')
        return 'waiting_for_pickup';
    if (value === 'handed_to_partner')
        return 'picked_up';
    if (value === 'failed')
        return 'cancelled';
    return value;
};
const mapOperationalStatusToStoredStatus = (status) => {
...
const deriveOperationalStatus = (order, trackingEvents = []) => {
...
const deriveDispatchState = (status) => {
    switch (status) {
        case 'pending_confirmation':
        case 'confirmed':
        case 'preparing':
...
const formatOperationalStatusLabel = (status) => normalizeOperationalStatus(status)
    .replace(/_/g, ' ')
    .replace(/\b\w/g, (char) => char.toUpperCase());
const summarizeOrderItems = (order) => Array.isArray(order?.items) && order.items.length
    ? order.items
        .map((item) => `${item.title || item.productId || 'Product'} x${Number(item.quantity || 1)}`)
        .join(', ')
    : order?.productId
        ? `${order.productId} x${Number(order.quantity || 1)}`
        : 'Marketplace item';
const toCurrency = (amount) => `AED ${Number(amount || 0).toFixed(2)}`;
const toDateTimeLabel = (value) => {
    const parsed = new Date(String(value || ''));
    return Number.isNaN(parsed.getTime()) ? 'Not available' : parsed.toLocaleString('en-AE');
};
const buildOrderFacts = (order, sellerName) => [
    ...
    { label: 'Order date', value: order.createdAt ? toDateTimeLabel(order.createdAt) : toDateTimeLabel(new Date().toISOString()) },
    {
        ...
        value: typeof order.shippingAddress === 'string'
...
const sendAdminAlert = async (subject, title, intro, facts = []) => {
...
const sendSellerNotice = async (sellerEmail, subject, title, intro, facts = []) => {
    await sendTransactionalEmail({
        to: sellerEmail,
        subject,
        title,
...
const sendCustomerNotice = async (customerEmail, subject, title, intro, facts = []) => {
    await sendTransactionalEmail({
        to: customerEmail,
        subject,
        title,
...
const hasAdminOrderNotificationEvent = (events = []) => events.some((event) => String(event?.status || '').toLowerCase() === 'admin_order_notified');
const notifyAdminsOfNewOrder = async (order, seller) => {
    try {
...
const ensureUniqueProductSlug = async (slugValue, currentId) => {
    ...
    return ensureUniqueSlug(slugValue || 'product', {
        currentId,
        exists: async (candidate) => (products || []).some((product) => slugifyProduct(product?.slug || product?.title || '') === candidate &&
            String(product?.id || '') !== String(currentId || '')),
    });
};
const buildPersistedProductSeo = async (input, currentId) => {
...
const getProductSeoMeta = (product) => {
...
const getProductCanonicalPath = (product) => {
    const canonicalAssignment = resolveCanonicalCategoryAssignment(product);
    const parentSlug = slugifyProduct(canonicalAssignment.parentCategorySlug ||
        product?.specs?.parentCategorySlug ||
        product?.specs?.categorySlug ||
        product?.category ||
        'electronics');
    const subcategorySlug = slugifyProduct(canonicalAssignment.subcategorySlug ||
...
const flattenCategoryIds = (categories) => {
    const ids = new Set();
...
const resolveEffectiveProductStatus = (product) => {
    ...
    if (productStatus === 'draft' || baseStatus === 'draft')
        return 'draft';
    if (productStatus === 'rejected' || approvalStatus === 'rejected' || baseStatus === 'rejected')
        return 'rejected';
    if (productStatus === 'archived' || baseStatus === 'archived')
        return 'archived';
    if (productStatus === 'live' || baseStatus === 'live')
        return 'live';
    if (productStatus === 'approved' || approvalStatus === 'approved' || baseStatus === 'approved')
        return 'approved';
    if (productStatus === 'pending_approval' || productStatus === 'pending' || approvalStatus === 'pending' || baseStatus === 'pending')
        return 'pending';
    return baseStatus || 'pending';
};
const isSoftDeletedProduct = (product) => {
    const specs = (product?.specs || product?.specsJson || {});
...
const buildSoftDeleteProductPayload = (product) => {
...
const deriveAdminLifecycle = (payload) => {
...
const assertAdminProductRequirements = (payload, lifecycleStatus) => {
...
const resolveAdminAssignedSeller = async (requestedSellerId) => {
...
const decorateCategory = (category) => {
    const interestCount = db
        .getAnalyticsEvents()
        .filter((event) => event.eventType === 'category_interest' &&
        (event.entityId === category.id || event.metadata?.categorySlug === category.slug)).length;
    return {
        ...category,
        active: typeof category.active === 'boolean' ? category.active : ACTIVE_CATEGORY_SLUGS.has(category.slug),
        comingSoonMessage: category.comingSoonMessage || "We're preparing amazing products in this category",
...
app.use(rateLimit({
    ...
    skip: (req) => {
        ...
        if (req.method !== 'GET')
            return false;
        return (requestPath === '/api/events/stream' ||
...
const buildHealthResponse = (req, res) => {
...
const setRefreshCookies = (res, refreshToken) => {
...
const parseAdminDateRange = (query) => {
...
const computeProductModerationSignals = (product, allProducts) => {
    ...
    const priceAnomaly = price <= 0 || price > 25000 || price < 10 || (product.originalPrice && price > Number(product.originalPrice));
    return {
        imageCount,
        titleQuality,
...
const serializeMarketplaceProduct = (product) => {
    ...
    const mergedSpecs = {
        ...(product.specs || {}),
        parentCategorySlug: canonicalAssignment.parentCategorySlug ||
            product.specs?.parentCategorySlug ||
            product.specs?.categorySlug ||
            '',
        categorySlug: canonicalAssignment.categorySlug ||
            product.specs?.categorySlug ||
            product.specs?.parentCategorySlug ||
            '',
        subcategorySlug: canonicalAssignment.subcategorySlug ||
            product.specs?.subcategorySlug ||
            product.specs?.templateId ||
            '',
        parentCategoryName: canonicalAssignment.parentCategoryName ||
            product.specs?.parentCategoryName ||
            '',
        categoryName: canonicalAssignment.categoryName ||
            product.specs?.categoryName ||
            '',
        subcategoryName: canonicalAssignment.subcategoryName ||
            product.specs?.subcategoryName ||
            product.specs?.templateName ||
            '',
        categoryPath: canonicalAssignment.categoryPath ||
...
const serializeMarketplaceOrder = (order) => {
...
const serializeMarketplaceOrderAsync = async (order) => {
    if (!prismaRuntime.enabled) {
...
const resolveCanonicalSellerForOrderItems = async (requestedSellerId, normalizedItems) => {
    const requested = String(requestedSellerId || '').trim();
    const productSellerIds = new Set();
    for (const item of normalizedItems) {
        ...
        const sellerId = String(product.sellerId || product.storeId || '').trim();
        if (!sellerId) {
...
const buildOrderPayloadFromItems = async ({ customerId, sellerId, normalizedItems, shippingCost, shippingAddress, customerName, customerEmail, customerPhone, paymentMethod, paymentProvider, paymentStatus, deliveryCountry, paymentReference, paymentSessionId, paidAt, }) => {
    ...
    const items = await Promise.all(normalizedItems.map(async (item, index) => {
        const product = prismaRuntime.enabled ? await prismaRuntime.getProduct(item.productId) : db.getProduct(item.productId);
        if (!product) {
            throw new Error(`Product not found: ${item.productId}`);
        }
        const productSellerId = String(product.sellerId || product.storeId || '').trim();
        ...
        if (item.variantId && Array.isArray(specs?.variants)) {
            variant = specs.variants.find((v) => String(v.id) === String(item.variantId) || String(v.id) === String(item.variantId || '')) || null;
            if (!variant) {
                // try matching by color+storage composite id if variantId provided in that form
                variant = specs.variants.find((v) => {
...
const buildSessionPayload = (userId) => {
...
const buildSessionPayloadAsync = async (userId) => {
    if (!prismaRuntime.enabled) {
...
const serializeMarketplaceProductAsync = async (product) => {
    if (!prismaRuntime.enabled) {
...
app.post('/api/auth/register', async (req, res) => {
    try {
        ...
        if (role === 'customer') {
            ...
            await sendCustomerNotice(user.email, 'Welcome to ExShopi', 'Your account is ready', 'Your ExShopi customer account has been created successfully.', [
...
app.post('/api/auth/login', async (req, res) => {
    try {
        ...
        if (!prismaRuntime.enabled) {
            ...
            await prismaRuntime.updateUser(user.id, { lastLoginAt: new Date().toISOString() });
        }
        const refreshToken = signRefreshToken({ id: user.id, role: user.role });
        setRefreshCookies(res, refreshToken);
        const session = await buildSessionPayloadAsync(user.id);
...
app.put('/api/users/:id', authMiddleware, (req, res) => {
    try {
        ...
        respond(db.updateUser(req.params.id, nextData));
    }
    catch (error) {
        res.status(500).json({ error: String(error) });
    }
...
// ==================== SELLERS API ====================
app.post('/api/sellers/create', authMiddleware, (req, res) => {
    try {
        ...
        const { storeName, description, logo, banner, city, country, phone, email, website = '', warehouseAddress = '', vatTrn = '', supportInfo = '', policies = {}, socialLinks = {}, bankAccount, bankName, accountHolder, iban = '', } = req.body;
...
app.get('/api/sellers/store/:storeSlug', async (req, res) => {
    try {
        const seller = prismaRuntime.enabled
...
app.put('/api/sellers/:id', authMiddleware, async (req, res) => {
    try {
        ...
        const updated = prismaRuntime.enabled
            ? await prismaRuntime.updateSeller(req.params.id, req.body)
            : db.updateSeller(req.params.id, req.body);
        res.json(updated);
    }
    catch (error) {
        res.status(500).json({ error: String(error) });
...
app.get('/api/seller/store/me', authMiddleware, async (req, res) => {
    try {
        const seller = prismaRuntime.enabled
...
// ==================== SELLER APPLICATIONS ====================
app.post('/api/seller-applications', authMiddleware, async (req, res) => {
    try {
        const user = prismaRuntime.enabled
        ...
        await sendAdminAlert(isResubmission ? 'Seller resubmission received' : 'New seller application received', isResubmission ? 'Seller resubmission ready for review' : 'New seller application ready for review', `${application.businessName} has been submitted to ExShopi and is waiting for admin review.`, [
...
app.get('/api/seller-applications/me', authMiddleware, async (req, res) => {
    try {
...
app.get('/api/admin/seller-applications', authMiddleware, async (req, res) => {
    try {
        ...
        const applications = await Promise.all(rawApplications.map(async (application) => ({
            ...application,
            user: sanitizeUser(prismaRuntime.enabled
...
app.post('/api/admin/seller-applications/:id/approve', authMiddleware, async (req, res) => {
    try {
        ...
        if (sellerUser?.email) {
            await sendSellerNotice(sellerUser.email, 'Your ExShopi seller account is approved', 'Seller approval confirmed', 'Your ExShopi seller application has been approved. You can now access the seller center and start submitting products.', [
...
app.post('/api/admin/seller-applications/:id/reject', authMiddleware, async (req, res) => {
    try {
        if (!isAdminLike(req.user?.role)) {
            return res.status(403).json({ error: 'Admin only' });
        }
        ...
        if (!application)
            return res.status(404).json({ error: 'Application not found' });
        if (prismaRuntime.enabled) {
            await prismaRuntime.updateUser(application.userId, { status: 'suspended', sellerApplicationStatus: 'rejected' });
        }
        else {
        ...
        if (sellerUser?.email) {
            await sendSellerNotice(sellerUser.email, 'ExShopi seller application update', 'Seller application rejected', 'Your seller application could not be approved at this time.', [
                { label: 'Store / business', value: application.businessName },
...
app.post('/api/admin/seller-applications/:id/request-info', authMiddleware, async (req, res) => {
    try {
        if (!isAdminLike(req.user?.role)) {
            return res.status(403).json({ error: 'Admin only' });
        }
        ...
        if (!application)
            return res.status(404).json({ error: 'Application not found' });
        if (prismaRuntime.enabled) {
            await prismaRuntime.updateUser(application.userId, { status: 'pending', sellerApplicationStatus: 'needs_more_info' });
        }
        else {
        ...
        if (sellerUser?.email) {
            await sendSellerNotice(sellerUser.email, 'More information needed for your seller application', 'Additional seller details requested', 'ExShopi needs a few more details before approving your seller account.', [
                { label: 'Store / business', value: application.businessName },
...
app.post('/api/products/create', authMiddleware, async (req, res) => {
    try {
        ...
        if (!isDraft) {
            ...
            await sendAdminAlert('New product submitted for review', 'Product review required', `${payload.title} is waiting for catalog review in ExShopi.`, [
...
app.get('/api/products/check-slug', async (req, res) => {
    try {
...
app.get('/api/products/:id', async (req, res) => {
    try {
        ...
        if (!product) {
            ...
            const all = prismaRuntime.enabled
                ...
                : supabaseRuntime.enabled
                    ...
                    : db.getAllProducts();
            product =
                all.find((p) => String(p.id) === param ||
                    String(p.slug || p.id) === param ||
                    seoSlugify(String(p.slug || p.title || p.id)) === seoSlugify(param)) || null;
            if (!product) {
        ...
        console.debug('[api] product found, returning', product.id || product.slug);
    ...
    try {
        ...
        if (categoryId) {
            if (prismaRuntime.enabled) {
                ...
                products = products.filter((product) => String(product.categoryId || product.specs?.backendCategoryId || '') === categoryId);
            }
            else if (supabaseRuntime.enabled) {
                const all = await supabaseRuntime.getAllProducts();
                products = all.filter((product) => String(product.categoryId || product.specs?.backendCategoryId || '') === categoryId);
            }
            else {
            ...
            const allProducts = prismaRuntime.enabled
                ...
                : supabaseRuntime.enabled
                    ? await supabaseRuntime.getAllProducts()
                    : db.getAllProducts();
            products = allProducts.filter((product) => productMatchesCategoryAssignment(product, categorySlug || null, subcategorySlug || null, parentSlug || null));
        }
        else {
    ...
    try {
        ...
        if (payload.specs || payload.title || payload.categoryId) {
            const normalizedSpecs = normalizeProductSpecifications({
                ...
                parentCategorySlug: payload.specs?.parentCategorySlug || current.specs?.parentCategorySlug || current.specs?.categorySlug || '',
                parentCategoryName: payload.specs?.parentCategoryName || current.specs?.parentCategoryName || current.specs?.categoryName || '',
        ...
        let product = null;
        if (prismaRuntime.enabled) {
            product = await prismaRuntime.updateProduct(req.params.id, payload);
        }
        else if (supabaseRuntime.enabled) {
            product = await supabaseRuntime.updateProduct(req.params.id, payload);
        }
        else {
            product = db.updateProduct(req.params.id, payload);
        }
        res.json(await serializeMarketplaceProductAsync(product));
    }
    catch (error) {
...
app.post('/api/products/:id/submit', authMiddleware, async (req, res) => {
    try {
        if (req.user?.role !== 'seller') {
        ...
        if (prismaRuntime.enabled) {
            updated = await prismaRuntime.updateProduct(req.params.id, updates);
        }
        else if (supabaseRuntime.enabled) {
            updated = await supabaseRuntime.updateProduct(req.params.id, updates);
        }
        else {
            updated = db.updateProduct(req.params.id, updates);
        }
        pushNotification({
            audience: 'admin',
            title: 'New product submitted',
        ...
        await sendAdminAlert('Product submitted for review', 'Product moderation queue updated', `${product.title} has been resubmitted and is waiting for admin review.`, [
...
app.post('/api/reviews', authMiddleware, async (req, res) => {
    try {
        if (req.user?.role !== 'customer')
            return res.status(403).json({ error: 'Only customers can post reviews' });
        ...
        if (prismaRuntime.enabled) {
            ...
            const eligibleOrder = deliveredOrders.find((order) => order.status === 'delivered' &&
...
app.post('/api/admin/products/:id/approve', authMiddleware, async (req, res) => {
    try {
        ...
        if (prismaRuntime.enabled) {
            product = await prismaRuntime.updateProduct(req.params.id, approveUpdates);
        }
        else if (supabaseRuntime.enabled) {
            product = await supabaseRuntime.updateProduct(req.params.id, approveUpdates);
        }
        else {
            product = db.updateProduct(req.params.id, approveUpdates);
        }
        if (!product) {
            return res.status(404).json({ error: 'Product not found' });
        }
        ...
        if (seller?.email) {
            ...
            await sendSellerNotice(seller.email, 'Your product is now live on ExShopi', 'Product approved', `${product.title} has been approved and is now visible to customers.`, [
                { label: 'Product', value: product.title },
...
app.post('/api/admin/products/:id/reject', authMiddleware, async (req, res) => {
    try {
        ...
        if (prismaRuntime.enabled) {
            product = await prismaRuntime.updateProduct(req.params.id, rejectUpdates);
        }
        else if (supabaseRuntime.enabled) {
            product = await supabaseRuntime.updateProduct(req.params.id, rejectUpdates);
        }
        else {
            product = db.updateProduct(req.params.id, rejectUpdates);
        }
        if (!product) {
            return res.status(404).json({ error: 'Product not found' });
        }
        ...
        if (seller?.email) {
            ...
            await sendSellerNotice(seller.email, 'Your product needs changes on ExShopi', 'Product rejected', `${product.title} was rejected during moderation.`, [
                { label: 'Product', value: product.title },
...
app.post('/api/admin/products/bulk-review', authMiddleware, async (req, res) => {
    try {
        if (!hasAdminPermission(req.user?.role, 'catalog:review')) {
            return res.status(403).json({ error: 'Catalog access only' });
        }
        ...
        for (const id of payload.ids) {
            const updates = payload.action === 'approve'
                ? {
                    status: 'live',
                    approvalStatus: 'approved',
                    productStatus: 'live',
            ...
            let updated = null;
            if (prismaRuntime.enabled) {
                updated = await prismaRuntime.updateProduct(id, updates);
            }
            else if (supabaseRuntime.enabled) {
                updated = await supabaseRuntime.updateProduct(id, updates);
            }
            else {
                updated = db.updateProduct(id, updates);
            }
            if (updated) {
                const seller = prismaRuntime.enabled ? await prismaRuntime.getSeller(updated.sellerId) : db.getSeller(updated.sellerId);
                if (seller?.email) {
                    const actionType = payload.action === 'approve'
                        ? 'product_approved'
                    ...
                    await sendSellerNotice(seller.email, payload.action === 'approve'
                        ? 'Your product is now live on ExShopi'
                        : payload.action === 'request_changes'
                            ? 'More information needed for your product'
                            : 'Your product was rejected on ExShopi', payload.action === 'approve'
                        ? 'Product approved'
                        : payload.action === 'request_changes'
                            ? 'Additional product details requested'
                            : 'Product rejected', payload.action === 'approve'
...
app.post('/api/admin/products', authMiddleware, async (req, res) => {
    try {
        ...
        const productInput = {
            ...
            description: payload.description || '',
            price: Number(payload.price || 0),
            originalPrice: payload.originalPrice != null ? Number(payload.originalPrice) : Number(payload.salePrice ?? payload.price ?? 0),
        ...
        let product = null;
        if (prismaRuntime.enabled) {
            product = await prismaRuntime.createProduct(productInput);
        }
        else if (supabaseRuntime.enabled) {
            product = await supabaseRuntime.createProduct(productInput);
        }
        else {
            product = db.createProduct(productInput);
        }
        res.json(await serializeMarketplaceProductAsync(product));
    }
    catch (error) {
...
app.put('/api/admin/products/:id', authMiddleware, async (req, res) => {
    try {
        if (!isAdminLike(req.user?.role))
            return res.status(403).json({ error: 'Admin only' });
        const nextPayload = { ...(await adminProductPayloadSchema.partial().parseAsync(req.body)) };
        ...
        if (nextPayload.specs || nextPayload.title || nextPayload.categoryId) {
            const normalizedSpecs = normalizeProductSpecifications({
                ...
                parentCategorySlug: nextPayload.specs?.parentCategorySlug || currentProduct?.specs?.parentCategorySlug || currentProduct?.specs?.categorySlug || '',
                parentCategoryName: nextPayload.specs?.parentCategoryName || currentProduct?.specs?.parentCategoryName || currentProduct?.specs?.categoryName || '',
                categorySlug: nextPayload.specs?.categorySlug || currentProduct?.specs?.categorySlug || currentProduct?.specs?.parentCategorySlug || '',
                categoryName: nextPayload.specs?.categoryName || currentProduct?.specs?.categoryName || currentProduct?.specs?.parentCategoryName || '',
                subcategorySlug: nextPayload.specs?.subcategorySlug || currentProduct?.specs?.subcategorySlug || currentProduct?.specs?.templateId || '',
                subcategoryName: nextPayload.specs?.subcategoryName || currentProduct?.specs?.subcategoryName || currentProduct?.specs?.templateName || '',
                title: nextPayload.title || currentProduct?.title,
            });
            nextPayload.specs = normalizedSpecs.specs;
            if ((nextPayload.status || currentProduct?.status) !== 'draft') {
        ...
        let updated = null;
        if (prismaRuntime.enabled) {
            updated = await prismaRuntime.updateProduct(req.params.id, nextPayload);
        }
        else if (supabaseRuntime.enabled) {
            updated = await supabaseRuntime.updateProduct(req.params.id, nextPayload);
        }
        else {
            updated = db.updateProduct(req.params.id, nextPayload);
        }
        if (!updated)
            return res.status(404).json({ error: 'Product not found' });
        res.json(await serializeMarketplaceProductAsync(updated));
...
app.delete('/api/admin/products/:id', authMiddleware, async (req, res) => {
    try {
        ...
        const deleted = prismaRuntime.enabled
            ? await prismaRuntime.updateProduct(req.params.id, updates)
            : supabaseRuntime.enabled
                ? await supabaseRuntime.updateProduct(req.params.id, updates)
...
// ==================== ORDERS API ====================
app.post('/api/cod/otp/send', authMiddleware, async (req, res) => {
    try {
        if (req.user?.role !== 'customer') {
...
app.post('/api/cod/otp/verify', authMiddleware, async (req, res) => {
    try {
        if (req.user?.role !== 'customer') {
            return res.status(403).json({ error: 'Customer only' });
        }
...
app.post('/api/orders/create', authMiddleware, async (req, res) => {
    try {
        ...
        const { sellerId, shippingCost, shippingAddress, customerName, customerEmail, customerPhone, verificationToken, paymentMethod, deliveryCountry, } = payload;
        const normalizedItems = payload.items && payload.items.length
            ? payload.items
            : payload.productId
        ...
        const priorCustomerOrdersToday = allOrders.filter((order) => {
            ...
            return (order.customerId === req.user.id &&
        ...
        await sendAdminAlert(risk.riskLevel === 'suspicious' ? 'Suspicious COD order placed' : 'New COD order placed', risk.riskLevel === 'suspicious' ? 'Suspicious COD order requires review' : 'New COD order received', `${customerName} placed a COD order on ExShopi.`, [
            ...buildOrderFacts({
                ...orderPayload,
        ...
        if (seller?.email) {
            await sendSellerNotice(seller.email, 'New order received in ExShopi Seller Center', 'A new order is ready for confirmation', `${customerName} has placed a COD order for your store.`, buildOrderFacts(orderPayload, seller.storeName));
        }
        if (customerEmail) {
            await sendCustomerNotice(customerEmail, 'Your ExShopi order has been placed', 'Order placed successfully', `Your Cash on Delivery order ${orderPayload.orderId} has been created and is now waiting for seller confirmation.`, buildOrderFacts(orderPayload, seller?.storeName));
        }
        let enrichedOrder;
        if (prismaRuntime.enabled) {
...
app.post('/api/payments/stripe/checkout-session', authMiddleware, async (req, res) => {
    try {
        if (req.user?.role !== 'customer') {
            return res.status(403).json({ error: 'Customer only' });
        }
        ...
        const lineItems = createdOrders.flatMap((order) => (order.items || []).map((item) => ({
            name: item.title,
...
app.post('/api/payments/stripe/webhook', async (req, res) => {
    try {
        const event = verifyStripeWebhookSignature(req.body, req.headers['stripe-signature']);
...
app.get('/api/orders/customer/:customerId', authMiddleware, async (req, res) => {
    try {
        ...
        res.json(prismaRuntime.enabled
...
app.get('/api/orders/seller/:sellerId', authMiddleware, async (req, res) => {
    try {
    ...
    try {
        ...
        if (prismaRuntime.enabled) {
            ...
            if (currentOrder.customerEmail) {
                await sendCustomerNotice(currentOrder.customerEmail, `Your ExShopi order is now ${formatOperationalStatusLabel(requestedStatus)}`, `Order ${formatOperationalStatusLabel(requestedStatus)}`, `${currentOrder.orderId || currentOrder.id} is now ${requestedStatus.replace(/_/g, ' ')}.`, buildOrderFacts(currentOrder, seller?.storeName));
            }
...
app.put('/api/orders/:id/request-return', authMiddleware, async (req, res) => {
    try {
        const order = prismaRuntime.enabled ? await prismaRuntime.getOrder(req.params.id) : db.getOrder(req.params.id);
        if (!order) {
            return res.status(404).json({ error: 'Order not found' });
        ...
        const next = prismaRuntime.enabled
            ? await prismaRuntime.updateOrder(req.params.id, {
                status: mapOperationalStatusToStoredStatus('return_requested'),
        ...
        if (prismaRuntime.enabled) {
            ...
            await sendAdminAlert('New return request received', 'Return request needs review', `${order.orderId || order.id} has a new return request from the customer.`, buildOrderFacts(order));
            if (order.customerEmail) {
                await sendCustomerNotice(order.customerEmail, 'Your return request has been submitted', 'Return request submitted', `We have received your return request for ${order.orderId || order.id}.`, buildOrderFacts(order));
            }
...
app.put('/api/orders/:id/dispatch-slot', authMiddleware, async (req, res) => {
    try {
        const order = prismaRuntime.enabled ? await prismaRuntime.getOrder(req.params.id) : db.getOrder(req.params.id);
        if (!order) {
            return res.status(404).json({ error: 'Order not found' });
        ...
        const updated = prismaRuntime.enabled
            ? await prismaRuntime.updateOrder(req.params.id, {
                ...
                status: mapOperationalStatusToStoredStatus(req.body.status || 'waiting_for_pickup'),
            })
        ...
        if (prismaRuntime.enabled) {
            ...
            if (order.customerEmail) {
                await sendCustomerNotice(order.customerEmail, 'Your order is waiting for pickup', 'Order waiting for pickup', `${order.orderId || order.id} is packed and waiting for ExShopi pickup.`, buildOrderFacts(order));
            }
    ...
    try {
        ...
        if (action === 'approve') {
            if (prismaRuntime.enabled) {
                ...
                if (order.customerEmail) {
                    await sendCustomerNotice(order.customerEmail, 'Your ExShopi return was approved', 'Return approved', `Your return for ${order.orderId || order.id} has been approved.`, buildOrderFacts(order));
                }
                return res.json(await serializeMarketplaceOrderAsync(updated));
            }
            ...
            if (prismaRuntime.enabled) {
                ...
                if (order.customerEmail) {
                    await sendCustomerNotice(order.customerEmail, 'Your ExShopi return was not approved', 'Return rejected', `Your return for ${order.orderId || order.id} could not be approved.`, [
...
app.post('/api/tracking/:orderId/scan-qr', authMiddleware, async (req, res) => {
    try {
        ...
        const order = prismaRuntime.enabled
            ? await prismaRuntime.updateOrder(req.params.orderId, { status: mapOperationalStatusToStoredStatus('picked_up') })
            : db.updateOrder(req.params.orderId, { status: mapOperationalStatusToStoredStatus('picked_up'), handedToCourierAt: new Date().toISOString() });
        if (!order) {
            return res.status(404).json({ error: 'Order not found' });
        }
        ...
        if (prismaRuntime.enabled) {
            ...
            if (orderBefore.customerEmail) {
                await sendCustomerNotice(orderBefore.customerEmail, 'Your ExShopi order was picked up', 'Order picked up', `${orderBefore.orderId || orderBefore.id} has been collected by ExShopi logistics.`, buildOrderFacts(orderBefore));
            }
            const seller = await prismaRuntime.getSeller(orderBefore.sellerId);
            if (seller?.email) {
                await sendSellerNotice(seller.email, 'Order picked up by ExShopi logistics', 'Order picked up', `${orderBefore.orderId || orderBefore.id} has been scanned and collected by ExShopi logistics.`, buildOrderFacts(orderBefore, seller.storeName));
            }
            return res.json(await serializeMarketplaceOrderAsync(order));
        }
...
app.post('/api/tracking/:orderId/delivery-done', authMiddleware, async (req, res) => {
    try {
        const order = prismaRuntime.enabled
            ? await prismaRuntime.updateOrder(req.params.orderId, {
                status: mapOperationalStatusToStoredStatus('delivered'),
        ...
        if (prismaRuntime.enabled) {
            ...
            if (order.customerEmail) {
                await sendCustomerNotice(order.customerEmail, 'Your ExShopi order was delivered', 'Order delivered', `${order.orderId || order.id} has been delivered successfully.`, buildOrderFacts(order));
            }
            return res.json(await serializeMarketplaceOrderAsync(order));
        }
...
// ==================== PAYOUTS API ====================
app.get('/api/payouts/seller/:sellerId', authMiddleware, async (req, res) => {
    try {
        const seller = prismaRuntime.enabled ? await prismaRuntime.getSellerByUserId(req.user.id) : db.getSellerByUserId(req.user.id);
        if (req.user?.role !== 'seller' || !seller || seller.id !== req.params.sellerId) {
            return res.status(403).json({ error: 'Seller ownership required' });
...
app.post('/api/admin/payouts/:id/process', authMiddleware, async (req, res) => {
    try {
        ...
        if (prismaRuntime.enabled) {
            ...
            if (seller?.email) {
                await sendSellerNotice(seller.email, 'Your ExShopi payout has been released', 'Payout released', 'Your latest ExShopi payout has been marked as released by finance.', [
        ...
        const relatedOrders = db.getSellerOrders(payout.sellerId).filter((order) => order.payoutStatus === 'processed' &&
            order.deliveredAt &&
...
app.post('/api/admin/payouts/generate', authMiddleware, async (req, res) => {
    try {
        if (!canAccessFinance(req.user?.role)) {
            return res.status(403).json({ error: 'Finance access only' });
        }
...
app.post('/api/payout-requests', authMiddleware, async (req, res) => {
    try {
        ...
        await sendAdminAlert('New payout request submitted', 'Payout request waiting for finance review', `${seller.storeName} has submitted a payout request in ExShopi.`, [
            { label: 'Seller', value: seller.storeName },
...
app.get('/api/payout-requests/seller/:sellerId', authMiddleware, async (req, res) => {
    try {
        if (req.user?.role !== 'seller' && !isAdminLike(req.user?.role))
            return res.status(403).json({ error: 'Forbidden' });
...
app.get('/api/admin/payout-requests', authMiddleware, async (req, res) => {
    try {
        if (!canAccessFinance(req.user?.role))
            return res.status(403).json({ error: 'Finance access only' });
        if (prismaRuntime.enabled) {
...
app.put('/api/admin/payout-requests/:id', authMiddleware, async (req, res) => {
    try {
        if (!canAccessFinance(req.user?.role))
            return res.status(403).json({ error: 'Finance access only' });
        if (prismaRuntime.enabled) {
...
app.get('/api/seller/analytics/:sellerId', authMiddleware, (req, res) => {
    try {
...
app.post('/api/categories/:id/interest', async (req, res) => {
    try {
        const categories = prismaRuntime.enabled
            ? await prismaRuntime.getCategories()
            : db.getCategories();
        const category = categories.find((entry) => entry.id === req.params.id || entry.slug === req.params.id);
        if (!category) {
...
app.put('/api/settings/site', authMiddleware, async (req, res) => {
    try {
        if (!isAdminLike(req.user?.role)) {
            return res.status(403).json({ error: 'Admin only' });
        }
...
app.get('/api/settings/marketplace', authMiddleware, async (req, res) => {
    try {
        if (!isAdminLike(req.user?.role)) {
            return res.status(403).json({ error: 'Admin only' });
        }
...
app.put('/api/settings/marketplace', authMiddleware, async (req, res) => {
    try {
        if (!isAdminLike(req.user?.role)) {
            return res.status(403).json({ error: 'Admin only' });
        }
...
app.get('/api/admin/activity-logs', authMiddleware, (req, res) => {
    try {
...
app.get('/api/support/tickets/me', authMiddleware, async (req, res) => {
    try {
        if (prismaRuntime.enabled) {
            return res.json(await prismaRuntime.getSupportTicketsForUser(req.user.id, req.user.role));
        }
        ...
        const tickets = db.getSupportTickets().filter((ticket) => {
            if (req.user?.role === 'seller')
                return ticket.sellerId === seller?.id || ticket.createdById === req.user.id;
            if (req.user?.role === 'customer')
                return ticket.customerId === req.user.id || ticket.createdById === req.user.id;
            return false;
        });
        res.json(tickets);
    }
...
app.put('/api/admin/support/tickets/:id', authMiddleware, async (req, res) => {
    try {
        if (!canAccessSupport(req.user?.role) && !isAdminLike(req.user?.role)) {
            return res.status(403).json({ error: 'Support access only' });
        }
...
app.post('/api/banners/:id/click', async (req, res) => {
    try {
        ...
        db.createAnalyticsEvent({
            ...
            role: (tryAuthenticateRequest(req)?.role || req.body.role || 'guest'),
...
app.get('/api/admin/banners/analytics', authMiddleware, async (req, res) => {
    try {
        if (!isAdminLike(req.user?.role)) {
            return res.status(403).json({ error: 'Admin only' });
        }
...
app.put('/api/banners/:id', authMiddleware, async (req, res) => {
    try {
        if (!isAdminLike(req.user?.role))
            return res.status(403).json({ error: 'Admin only' });
        const updated = prismaRuntime.enabled
            ? await prismaRuntime.updateBanner(req.params.id, req.body)
            : db.updateBanner(req.params.id, req.body);
        if (!updated)
            return res.status(404).json({ error: 'Banner not found' });
        res.json(updated);
    }
...
app.put('/api/categories/:id', authMiddleware, (req, res) => {
    try {
        ...
        const updated = db.updateCategory(req.params.id, normalized);
        if (!updated)
            return res.status(404).json({ error: 'Category not found' });
        res.json(updated);
    }
...
app.get('/sitemap.xml', async (_req, res) => {
    try {
        ...
        for (const category of categories || []) {
            if (!category?.slug)
                continue;
            urls.set(`${siteUrl}/category/${escapeXml(String(category.slug))}`, getSitemapMeta(`/category/${String(category.slug)}`));
            for (const sub of category?.subcategories || []) {
                if (!sub?.slug)
                    continue;
                urls.set(`${siteUrl}/category/${escapeXml(String(category.slug))}/${escapeXml(String(sub.slug))}`, getSitemapMeta(`/category/${String(category.slug)}/${String(sub.slug)}`));
            }
        }
        for (const product of products || []) {
            if (isSoftDeletedProduct(product))
                continue;
            ...
            if (!(status === 'live' || (approval === 'approved' && visibility === 'live')))
                continue;
...
${Array.from(urls.entries())
            ...
            .map(([url, meta]) => `  <url><loc>${url}</loc><lastmod>${lastmod}</lastmod><changefreq>${meta.changefreq}</changefreq><priority>${meta.priority}</priority></url>`)
            .join('\n')}
</urlset>`;
...
app.get('/api/admin/dashboard', authMiddleware, async (req, res) => {
    try {
        ...
        const [sellers, allProducts, orders, payoutRequests, payouts, users, supportTickets, banners,] = prismaRuntime.enabled
            ? await Promise.all([
        ...
        const fraudFlagOrders = ordersInRange.filter((order) => String(order.notes || '').toLowerCase().includes('fraud') ||
            String(order.cancellationReason || '').toLowerCase().includes('fraud'));
        const openSupportCases = supportInRange.filter((ticket) => !['resolved', 'closed'].includes(String(ticket.status)));
        const totalCustomers = users.filter((user) => user.role === 'customer').length;
        const customerOrderCounts = ordersInRange.reduce((acc, order) => {
            if (!order.customerId)
                return acc;
        ...
        const repeatPurchaseRate = ordersInRange.length
            ? Math.round((Object.values(customerOrderCounts).filter((count) => count > 1).length / Math.max(1, Object.keys(customerOrderCounts).length)) *
                100)
            : 0;
        ...
        const bucketSizeDays = dateRange.range === 'today'
            ? 1
        ...
        const alerts = [
            ...
            },
            {
                ...
                severity: ordersInRange.length > 0 && (returnRequestedOrders.length + returnedOrders.length) / ordersInRange.length > 0.08
                    ? 'high'
                    : 'medium',
                to: '/admin/returns',
            },
...
app.get('/api/seller/dashboard/:sellerId', authMiddleware, async (req, res) => {
    try {
...
if (process.env.NODE_ENV !== 'production') {
    app.post('/__dev/admin/update-product', async (req, res) => {
        try {
...
const startServer = async () => {
    if (prismaRuntime.enabled) {
        try {
            ...
            if (bundledImport.attempted) {
                console.log(`[BOOT] Bundled draft import summary: total=${bundledImport.totalRows} imported=${bundledImport.imported} duplicates=${bundledImport.duplicates} failed=${bundledImport.failed}`);
            }
            else {
    ...
    app.listen(PORT, () => {
        ...
        console.log(`[BOOT] DB config summary: mode=${connectionMode} databaseHost=${prismaEnvDiagnostics.databaseUrlHost} directHost=${prismaEnvDiagnostics.directUrlHost}`);
        ...
        if (prismaRuntime.enabled) {
            void probePrismaConnection().then((result) => {
                ...
                console.error(`[DB] Prisma connection probe failed (${result.name}/${result.code}): ${result.message}`);
                ...
                console.error('[DB] Expected format: DATABASE_URL=postgresql://postgres.<project-ref>:<db-password>@<region>.pooler.supabase.com:6543/postgres?pgbouncer=true and DIRECT_URL=postgresql://postgres:<db-password>@db.<project-ref>.supabase.co:5432/postgres');
            });
        }
    });
};
