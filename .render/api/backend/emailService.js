...
function renderEmailHtml(input) {
    const facts = (input.facts || [])
        ...
        .map((fact) => `<tr><td style="padding:8px 12px;color:#64748b;font-size:13px;border-bottom:1px solid #e2e8f0;">${escapeHtml(fact.label)}</td><td style="padding:8px 12px;color:#0f172a;font-size:13px;font-weight:600;border-bottom:1px solid #e2e8f0;">${escapeHtml(fact.value)}</td></tr>`)
        .join('');
    const bullets = (input.bullets || [])
        .filter(Boolean)
        .map((bullet) => `<li style="margin:0 0 8px 0;color:#334155;font-size:14px;line-height:1.6;">${escapeHtml(bullet)}</li>`)
        .join('');
    const cta = input.ctaLabel && input.ctaUrl
        ? `<div style="margin:24px 0 0;"><a href="${escapeHtml(input.ctaUrl)}" style="display:inline-block;padding:12px 18px;border-radius:12px;background:#2563eb;color:#ffffff;text-decoration:none;font-weight:600;">${escapeHtml(input.ctaLabel)}</a></div>`
        : '';
    return `
