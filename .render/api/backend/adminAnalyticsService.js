...
function isBetweenDates(value, from, to) {
    if (!value)
        return false;
...
function resolveOrderProductId(item, order) {
    return String(item?.productId || item?.id || order?.productId || '').trim();
}
function resolveOrderProductTitle(item, order) {
...
function normalizeChannel(value) {
    ...
    if (raw.includes('organic'))
        return 'organic';
    if (raw.includes('social'))
        return 'social';
    if (raw.includes('referral'))
        return 'referral';
...
export async function buildStoreOperationsAnalytics(dateRange) {
    const [products, orders, sellers, payoutRequests, payouts, users, analyticsEvents,] = await (prismaRuntime.enabled
        ? Promise.all([
    ...
    const pendingOrders = ordersInRange.filter((order) => ['pending_confirmation', 'placed', 'confirmed', 'preparing', 'packed', 'waiting_for_pickup'].includes(normalizeValue(order.status)));
    const shippedOrders = ordersInRange.filter((order) => ['picked_up', 'in_transit', 'out_for_delivery', 'shipped'].includes(normalizeValue(order.status)));
    ...
    ordersInRange.forEach((order) => {
        ...
        if (!lineItems.length) {
            ...
            const fallbackTitle = String(order.productTitle || 'Marketplace Product').trim();
    ...
    const sellerPerformance = sellers
        ...
        const returns = sellerOrders.filter((order) => ['return_requested', 'returned', 'refunded'].includes(normalizeValue(order.status)) ||
            ['requested', 'refunded'].includes(normalizeValue(order.refundStatus))).length;
        return {
            sellerId: seller.id,
            ...
            payoutDue: sumBy(sellerOrders.filter((order) => normalizeValue(order.payoutStatus) !== 'paid'), (order) => Number(order.sellerAmount || 0)),
        };
    })
    ...
    const payoutSummary = {
        ...
        totalPendingAmount: sumBy(payoutRequests.filter((request) => normalizeValue(request.status) === 'pending'), (request) => Number(request.amount || 0)),
        paidThisMonth: sumBy(payouts.filter((payout) => normalizeValue(payout.status) === 'paid' && isBetweenDates(payout.createdAt, monthStart, todayEnd)), (payout) => Number(payout.netAmount || 0)),
        processedThisMonth: sumBy(payouts.filter((payout) => normalizeValue(payout.status) === 'processed' && isBetweenDates(payout.createdAt, monthStart, todayEnd)), (payout) => Number(payout.netAmount || 0)),
        payoutDue: sumBy(orders.filter((order) => normalizeValue(order.payoutStatus) !== 'paid'), (order) => Number(order.sellerAmount || 0)),
    };
    const topSearches = Object.entries(analyticsEvents
    ...
    const mostViewedProducts = Object.entries(analyticsEvents
        ...
        .slice(0, 10);
    const mostWishlistedProducts = Object.entries(analyticsEvents
        ...
        .slice(0, 10);
    const acquisitionChannels = Object.entries(analyticsEvents
        .filter((event) => isBetweenDates(event.createdAt, dateRange.from, dateRange.to))
        .reduce((accumulator, event) => {
        const inferredSource = String(event.metadata?.source ||
