...
const enabled = process.env.USE_PRISMA_RUNTIME === 'true' ||
...
async function hashSeedPasswordIfNeeded(password) {
...
function mapSellerApplication(application) {
    return {
        id: application.id,
...
function mapSeller(store) {
    return {
        ...
        policies: store.policiesJson || {},
        socialLinks: store.socialLinksJson || {},
...
function mapProduct(product) {
    ...
    const primaryImage = images.find((image) => image.isPrimary)?.imageUrl ||
        orderedImages[0] ||
        '';
    ...
    if (!specs.metaTitle && product.metaTitle)
        specs.metaTitle = product.metaTitle;
    if (!specs.metaDescription && product.metaDescription)
        specs.metaDescription = product.metaDescription;
    if (!specs.metaKeywords && product.metaKeywords)
        specs.metaKeywords = product.metaKeywords;
    if (!specs.canonicalUrl && product.canonicalUrl)
        specs.canonicalUrl = product.canonicalUrl;
    if (!specs.ogTitle && product.ogTitle)
        specs.ogTitle = product.ogTitle;
    if (!specs.ogDescription && product.ogDescription)
        specs.ogDescription = product.ogDescription;
    if (!specs.ogImage && product.ogImage)
        specs.ogImage = product.ogImage;
    if (!specs.parentCategorySlug && product.parentCategorySlug)
        specs.parentCategorySlug = product.parentCategorySlug;
    if (!specs.categorySlug && product.categorySlug)
        specs.categorySlug = product.categorySlug;
    if (!specs.subcategorySlug && product.subcategorySlug)
        specs.subcategorySlug = product.subcategorySlug;
    if (!specs.childCategorySlug && product.childCategorySlug)
        specs.childCategorySlug = product.childCategorySlug;
    if (!specs.categoryPath && product.categoryPathJson)
        specs.categoryPath = product.categoryPathJson;
    return {
        id: product.id,
        slug: product.slug || "",
        ...
        ownership: (product.ownership || 'seller'),
        createdByRole: (product.createdByRole || 'seller'),
...
function withDeletionMeta(specs, nextMeta) {
    return {
        ...(specs || {}),
        __deletion: {
            ...((specs || {}).__deletion || {}),
...
function isSoftDeletedProduct(product) {
...
function mapOrderItem(item) {
    return {
        ...
        sku: item.sku || '',
        image: item.product?.images?.find((image) => image.isPrimary)?.imageUrl || item.product?.images?.[0]?.imageUrl || '',
    };
}
...
function mapPayout(payout, seller) {
...
function mapPayoutRequest(request, seller) {
    return {
        id: request.id,
        sellerId: seller?.id || '',
        ...
        notes: request.notes || '',
        bankDetails: request.bankDetailsJson || {},
...
function mapSupportMessage(message) {
    return {
        id: message.id,
...
function mapSupportTicket(ticket) {
    return {
        ...
        createdByRole: (ticket.createdByRole || 'customer'),
...
export const prismaRuntime = {
    ...
    async getSiteSettings() {
        ...
        return record?.valueJson || db.getSiteSettings();
    },
    async updateSiteSettings(settings) {
        if (!enabled)
            return null;
        const current = (await prisma.setting.findUnique({ where: { key: 'site_settings' } }))?.valueJson ||
            db.getSiteSettings();
        ...
        await prisma.setting.upsert({
            ...
            create: { key: 'site_settings', valueJson: merged },
        });
        return merged;
    },
    async getMarketplaceSettings() {
        ...
        return record?.valueJson || db.getMarketplaceSettings();
    },
    async updateMarketplaceSettings(settings) {
        if (!enabled)
            return null;
        const current = (await prisma.setting.findUnique({ where: { key: 'marketplace_settings' } }))?.valueJson ||
        ...
        await prisma.setting.upsert({
            ...
            update: { valueJson: merged },
            create: { key: 'marketplace_settings', valueJson: merged },
        });
        return merged;
    },
    async getBanners() {
        ...
        const banners = record?.valueJson || fallback;
    ...
    async saveBanners(banners) {
        ...
        await prisma.setting.upsert({
            where: { key: 'banners' },
            update: { valueJson: banners },
            create: { key: 'banners', valueJson: banners },
        });
        return banners;
    },
    ...
    async updateBanner(id, updates) {
        if (!enabled)
            return null;
        const banners = await this.getBanners();
        const next = banners.map((banner) => banner.id === id ? { ...banner, ...updates, updatedAt: new Date().toISOString() } : banner);
    ...
    async incrementBannerClicks(id) {
        if (!enabled)
            return null;
        const banners = await this.getBanners();
        const next = banners.map((banner) => banner.id === id
    ...
    async createUser(input) {
        ...
        const user = await prisma.user.create({
            data: {
                ...
                sellerApplicationStatus: (input.sellerApplicationStatus || null),
            },
        });
        return mapUser(user);
    },
        ...
        if (existingOfficialStore) {
            await prisma.store.update({
                ...
                data: {
                    ...
                    description: existingOfficialStore.description ||
    ...
    async ensureBundledDraftProductsImported() {
        ...
        const officialStore = (await this.getSeller(CORE_OFFICIAL_STORE_ID)) ||
        ...
        const existingSlugs = new Set(existingProducts.map((product) => String(product.slug || '').trim().toLowerCase()).filter(Boolean));
        const existingTitles = new Set(existingProducts.map((product) => String(product.title || '').trim().toLowerCase()).filter(Boolean));
        const existingBrandModels = new Set(existingProducts
            .map((product) => {
        ...
        for (const product of bundledProducts) {
            ...
            try {
                ...
                if (brandModelKey)
                    existingBrandModels.add(brandModelKey);
                imported += 1;
            }
            catch (error) {
                failed += 1;
    ...
    async updateUser(id, data) {
        ...
        const user = await prisma.user.update({
            ...
            data: {
                ...
                sellerApplicationStatus: (data.sellerApplicationStatus || undefined),
    ...
    async updateSeller(id, updates) {
        ...
        const store = await prisma.store.update({
            ...
            data: {
                ...
                iban: updates.iban,
                policiesJson: updates.policies,
                socialLinksJson: updates.socialLinks,
    ...
    async createSellerApplication(input) {
        if (!enabled)
            return null;
    ...
    async updateSellerApplication(id, updates) {
        if (!enabled)
            return null;
    ...
    async ensureSellerFromApplication(applicationId) {
        if (!enabled)
            return null;
        const application = await prisma.sellerApplication.findUnique({
    ...
    async createProduct(input) {
        ...
        const product = await prisma.product.create({
            data: {
                ...
                sellerUserId: (await prisma.store.findUnique({
                ...
                status: finalStatus,
                visibilityStatus: finalVisibilityStatus,
                approvalRequestedAt: input.approvalRequestedAt ? new Date(input.approvalRequestedAt) : new Date(),
                rejectionReason: "",
                approvalNotes: "",
                approvedAt: finalApprovalStatus === 'approved' ? new Date() : undefined,
    ...
    async getProductsByCategorySlugs(parentSlug, categorySlug, subcategorySlug) {
        ...
        if (categorySlug && !parentSlug && !subcategorySlug) {
            ...
            const andClauses = [];
            if (parentSlug)
                andClauses.push({ parentCategorySlug: parentSlug });
            if (categorySlug)
                andClauses.push({ categorySlug });
            if (subcategorySlug)
                andClauses.push({ subcategorySlug });
    ...
    async updateProduct(id, updates) {
        ...
        const nextSpecs = withDeletionMeta({
            ...
            isDeleted: updates.isDeleted ?? ((current.specsJson || {}).__deletion?.isDeleted) ?? false,
            deletedAt: updates.deletedAt ?? ((current.specsJson || {}).__deletion?.deletedAt) ?? '',
        });
        if (nextSpecs) {
        ...
        const product = await prisma.product.update({
            ...
            data: {
                ...
                status: ((updates.productStatus || updates.status) === 'pending' ? 'pending_approval' : (updates.productStatus || updates.status || undefined)),
                visibilityStatus: updates.visibilityStatus,
                rejectionReason: updates.rejectionReason,
                ...
                images: updates.image || updates.images
                    ? {
    ...
    async deleteProduct(id) {
        ...
        if (!current)
            return false;
        const specs = withDeletionMeta(current.specsJson || {}, {
            isDeleted: true,
    ...
    async addTrackingEvent(orderId, status, timestamp, notes, location) {
        if (!enabled)
            return null;
    ...
    async getReviewByOrderItem(orderItemId, customerId) {
        if (!enabled)
            return null;
    ...
    async getOrderItemWithOrder(orderItemId) {
        if (!enabled)
            return null;
    ...
    async getVendorReviews(storeId) {
        if (!enabled)
            return [];
        const reviews = await prisma.review.findMany({
    ...
    async getSellerPayouts(storeId) {
        if (!enabled)
            return [];
        const store = await prisma.store.findUnique({ where: { id: storeId } });
        if (!store)
        ...
        return payouts.map((payout) => mapPayout(payout, store));
    },
    async getAllPayouts() {
        if (!enabled)
            return [];
    ...
    async createPayoutRequest(input) {
        ...
        return mapPayoutRequest(request, store);
    },
    async getSellerPayoutRequests(storeId) {
        if (!enabled)
            return [];
        const store = await prisma.store.findUnique({ where: { id: storeId } });
        if (!store)
        ...
        return requests.map((request) => mapPayoutRequest(request, store));
    },
    ...
    async addSupportTicketMessage(id, message) {
        if (!enabled)
            return null;
