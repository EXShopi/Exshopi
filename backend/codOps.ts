...
export type CodRiskLevel = 'normal' | 'suspicious';
export type BlacklistType = 'phone' | 'email' | 'ip';
export type NotificationAudience = 'admin' | 'seller' | 'customer';
...
type CodState = {
  otpSessions: CodOtpSession[];
  blacklist: BlacklistEntry[];
  notifications: CodNotification[];
};
...
function saveState(state: CodState) {
...
export function createCodOtpSession(input: {
  ...
  const existingRecent = state.otpSessions
    .filter(
      (session) =>
        session.phone === normalizePhone(input.phone) &&
...
export function verifyCodOtpSession(sessionId: string, code: string) {
  const state = loadState();
  const session = state.otpSessions.find((item) => item.id === sessionId);
  if (!session) throw new Error('OTP session not found.');
  if (session.usedAt) throw new Error('This OTP session was already used.');
  if (new Date(session.expiresAt).getTime() < Date.now()) throw new Error('OTP expired. Please resend.');
  if (session.attempts >= 5) throw new Error('Too many OTP attempts.');
...
export function consumeCodOtpVerification(sessionId: string, customerId: string, phone: string) {
  const state = loadState();
  const session = state.otpSessions.find((item) => item.id === sessionId);
  if (!session?.verifiedAt) throw new Error('Order phone verification is missing.');
  if (session.customerId !== customerId) throw new Error('OTP verification does not match the signed-in customer.');
  if (session.phone !== normalizePhone(phone)) throw new Error('Verified phone does not match the checkout phone.');
  if (new Date(session.expiresAt).getTime() < Date.now()) throw new Error('Verified OTP expired. Please try again.');
...
export function addBlacklistEntry(input: {
  ...
  const normalizedValue =
    input.type === 'phone' ? normalizePhone(input.value) : String(input.value || '').trim().toLowerCase();
...
export function isBlacklisted(input: { phone?: string; email?: string; ip?: string }) {
  const state = loadState();
...
export function pushNotification(input: Omit<CodNotification, 'id' | 'createdAt'>) {
  const state = loadState();
  const notification: CodNotification = {
    ...input,
...
export function getNotificationsForAudience(audience: NotificationAudience, audienceId?: string) {
  const state = loadState();
  return state.notifications.filter(
    (item) => item.audience === audience && (!item.audienceId || !audienceId || item.audienceId === audienceId)
  );
}
...
export function calculateCodRisk(input: {
  ...
  email: string;
  ip: string;
  orders: Array<{ customerPhone?: string; customerEmail?: string; customerId?: string; createdAt?: string; status?: string }>;
  customerId: string;
}) {
  const now = Date.now();
  ...
  if (phoneOrders >= 3) reasons.push('High daily order count from the same phone');
  if (emailOrders >= 4) reasons.push('High daily order count from the same email');
  if (customerOrders >= 4) reasons.push('Customer exceeded normal COD daily activity');

  return {
    riskLevel: reasons.length ? ('suspicious' as CodRiskLevel) : ('normal' as CodRiskLevel),
    reasons,
