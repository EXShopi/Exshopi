...
import { BarChart3, Bell, ChevronRight, HelpCircle, Image as ImageIcon, Layers, LayoutDashboard, LogOut, Menu, Package, Percent, Search, Settings, ShoppingBag, ShoppingCart, Tag, UserCircle, Users, Wallet, X, RotateCcw, ShieldCheck } from 'lucide-react';
...
const canAccessAdmin = runtimeRole === 'admin' || runtimeRole === 'super_admin' || runtimeRole === 'finance_manager' || runtimeRole === 'support_agent';
...
if (hasPersistedAuth && !canAccessAdmin) {
navigate('/admin/login', { replace: true });
setIsAuthLoading(false);
return;
}
...
if (isAuthLoading) {
return (
<div className="min-h-screen bg-[radial-gradient(circle_at_top_left,_rgba(59,130,246,0.16),_transparent_24%),radial-gradient(circle_at_top_right,_rgba(168,85,247,0.12),_transparent_24%),linear-gradient(to_bottom,transparent,rgba(15,23,42,0.5))] flex items-center justify-center">
<div className="text-center">
<div className="inline-flex h-16 w-16 animate-spin rounded-full border-4 border-slate-200 border-t-violet-600"></div>
<p className="mt-6 text-slate-600 font-medium">Verifying admin access...</p>
</div>
</div>
);
}
...
const activeItem = useMemo(() => menuItems.find((item) => location.pathname.startsWith(item.path)), [location.pathname]);
...
return (
<div className="min-h-screen bg-[radial-gradient(circle_at_top_left,_rgba(59,130,246,0.16),_transparent_24%),radial-gradient(circle_at_top_right,_rgba(168,85,247,0.12),_transparent_24%),linear-gradient(to_bottom,transparent,rgba(15,23,42,0.5))]">
{isSidebarOpen && (
<div className="fixed inset-0 z-40 bg-slate-950/50 backdrop-blur-md lg:hidden" onClick={() => setIsSidebarOpen(false)} />
)}
<div className="flex min-h-screen">
<aside className={`fixed inset-y-0 left-0 z-50 w-[308px] transform border-r border-white/50 bg-slate-950 text-white transition-transform duration-300 lg:relative lg:translate-x-0 ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'}`} >
...
return (
<Link key={item.label} to={item.path} className={`group flex items-center gap-3 rounded-2xl px-4 py-3.5 text-sm font-bold transition-all ${isActive ? 'bg-gradient-to-r from-blue-600 via-violet-600 to-cyan-500 text-white shadow-lg shadow-blue-500/20' : 'text-slate-400 hover:bg-white/5 hover:text-white'}`}>
{item.icon ? <item.icon size={19} /> : null}
<span>{item.label}</span>
<ChevronRight size={16} className={`ml-auto transition-transform ${isActive ? 'opacity-100' : 'opacity-0 group-hover:translate-x-0.5 group-hover:opacity-100'}`} />
</Link>
);
})}
</div>
...
<button onClick={handleLogout} className="flex w-full items-center gap-3 rounded-2xl px-4 py-3 text-sm font-bold text-rose-400 transition hover:bg-rose-500/10">
...
<p className="text-sm font-black text-slate-900"> {getAdminRoleLabel(effectiveRole)} </p>
<p className="text-[10px] font-black uppercase tracking-[0.22em] text-violet-600">{user?.email}</p>
</div>
<div className="flex h-11 w-11 items-center justify-center rounded-2xl bg-gradient-to-br from-slate-950 via-slate-800 to-blue-600 font-black text-white">
...
>>>>>>> e42e511 (Fix AdminLayout syntax error)
}