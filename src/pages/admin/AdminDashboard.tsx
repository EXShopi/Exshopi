...
export default function AdminDashboard() {
  ...
  const pendingApprovals =
    dashboardData.pendingProducts ?? dashboardData.metrics?.pendingProducts ?? 0;
  ...
  const totalCommission =
    dashboardData.platformCommission ?? dashboardData.metrics?.platformCommission ?? 0;
  const vendorPayouts = dashboardData.vendorPayouts ?? dashboardData.metrics?.vendorPayouts ?? 0;
  const returnRequestedOrders =
    dashboardData.returnRequestedOrders ?? dashboardData.metrics?.returnRequests ?? 0;
  const pendingPayoutRequests =
    dashboardData.pendingPayoutRequests ?? dashboardData.metrics?.pendingPayoutRequests ?? 0;
  const lowStockProducts =
    dashboardData.lowStockProducts ?? dashboardData.metrics?.lowStockProducts ?? 0;
  ...
  const openSupportCases =
    dashboardData.openSupportCases ?? dashboardData.metrics?.openSupportCases ?? 0;
  const marketplaceHealthScore = Number(
    dashboardData.marketplaceHealthScore ??
      dashboardData.metrics?.marketplaceHealthScore ??
      0
  );
  ...
  const searchKeywords = Array.isArray(dashboardData.searchKeywords)
    ? dashboardData.searchKeywords
    : [];
  ...
  const topSellers = Array.isArray(dashboardData.topSellers) ? dashboardData.topSellers : [];
  ...
  const operationalCards = [
    {
      ...
      text: 'Seller and product queues waiting for action',
    },
    {
      label: 'Pending Orders',
      value: pendingOrders,
      text: 'Marketplace orders awaiting fulfillment',
    },
    {
      label: 'Return Requests',
      value: returnRequestedOrders,
      text: 'Refund and after-sales cases under review',
    },
    {
      label: 'Low Stock Products',
      value: lowStockProducts,
      text: 'Inventory alerts requiring immediate action',
    },
  ];
  ...
  const actionTiles = [
    {
      ...
      text: 'Review seller applications and product submissions.',
      to: '/admin/approvals',
    },
    {
      title: 'Manage Vendors',
      text: 'Audit seller quality, documents, and store health.',
      to: '/admin/vendors',
    },
    {
      title: 'Customers',
      text: 'Open customer intelligence, engagement, and buyer behavior.',
      to: '/admin/customers',
    },
    {
      title: 'Returns & Refunds',
      text: 'Process after-sales requests, return reasons, and refund status.',
      to: '/admin/returns',
    },
    {
      title: 'Control Homepage',
      text: 'Update banners, offers, sections, and campaign visuals.',
      to: '/admin/banners',
    },
    {
      title: 'Finance & Payouts',
      text: 'Track commissions, liabilities, and payout requests.',
      to: '/admin/payouts',
    },
    {
      title: 'Add ExShopi Product',
      text: 'Create and publish official ExShopi listings using the structured marketplace form.',
    ...
    <div className="space-y-8">
      ...
      <section className="grid gap-6 xl:grid-cols-[1.25fr_0.75fr]">
        <div className="rounded-[36px] border border-blue-100 bg-gradient-to-br from-slate-950 via-[#172554] to-[#2563eb] p-8 text-white shadow-2xl shadow-blue-500/15">
          ...
          <h2 className="mt-5 text-4xl font-black tracking-tight">
            ExShopi Admin Command Layer
          </h2>
          <p className="mt-3 max-w-2xl text-sm leading-7 text-blue-100/90">
            A premium UAE-first marketplace cockpit for approvals, sellers, homepage
            content, finance, and live commerce performance across ExShopi Official and
            third-party vendors.
          </p>
          ...
          <div className="mt-6 grid gap-4 sm:grid-cols-3">
            {[
              ...
              <div
                key={label}
                className="rounded-2xl border border-white/10 bg-slate-950/20 px-4 py-4 backdrop-blur"
              >
                <p className="text-[10px] font-black uppercase tracking-[0.22em] text-blue-200">
                  {label}
                </p>
      ...
      <section className="grid gap-5 md:grid-cols-2 xl:grid-cols-4">
        {primaryCards.map((card) => (
          <div
            key={card.label}
            className={`rounded-[30px] border border-slate-200 bg-gradient-to-br ${card.tone} p-6 shadow-sm`}
          >
          ...
          <div className="mb-5 flex items-center gap-3">
            ...
            <div>
              ...
              <p className="mt-1 text-sm font-medium text-slate-500">
                Live operational risks and moderation queues across ExShopi.
              </p>
            </div>
          </div>
        ...
        <div className="rounded-[30px] border border-slate-200 bg-white p-6 shadow-sm">
          <div className="mb-5 flex items-center gap-3">
            ...
            <div>
              ...
              <p className="mt-1 text-sm font-medium text-slate-500">
                Operational depth for growth, trust, and buyer retention.
              </p>
            </div>
          </div>
          <div className="space-y-3">
      ...
      <section className="grid gap-6 xl:grid-cols-[1.15fr_0.85fr]">
        <div className="rounded-[34px] border border-slate-200 bg-white p-7 shadow-sm">
          <div className="mb-6 flex items-center justify-between">
            <div>
              ...
              <p className="mt-1 text-sm font-medium text-slate-500">
                Marketplace GMV and ExShopi take-rate movement across the last six periods.
              </p>
            </div>
          </div>
        ...
        <div className="rounded-[34px] border border-slate-200 bg-white p-7 shadow-sm">
          <div className="mb-4">
            ...
            <p className="mt-1 text-sm font-medium text-slate-500">
              Operational status distribution across active marketplace orders.
            </p>
          </div>
        ...
        <div className="rounded-[34px] border border-slate-200 bg-white p-7 shadow-sm">
          <div className="mb-5 flex items-center justify-between">
            <div>
              ...
              <p className="mt-1 text-sm font-medium text-slate-500">
                A premium high-level seller ranking to guide operations and merchandising.
              </p>
            </div>
            <button
          ...
          <div className="space-y-3">
            {topSellers.length ? (
              topSellers.map((seller: any) => (
                <div
                  key={seller.name}
                  className="grid gap-4 rounded-[24px] border border-slate-200 p-4 md:grid-cols-[1.4fr_0.7fr_0.7fr_0.6fr] md:items-center"
                >
                  <div>
                  ...
                  <div>
                    ...
                    <p className="mt-1 text-lg font-black text-slate-900">
                      {formatAED(Number(seller.sales || 0))}
                    </p>
                  </div>
                  <div>
                    <p className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-400">
                    ...
                    <p className="mt-1 text-lg font-black text-amber-600">
                      {formatAED(Number(seller.commission || 0))}
                    </p>
                  </div>
