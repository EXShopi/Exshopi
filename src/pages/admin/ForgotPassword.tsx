  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (result?.success) {
        ...
        setTimeout(() => {
          navigate(`/admin/reset-password?email=${encodeURIComponent(email.trim())}`);
        }, 2000);
      } else {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 flex items-center justify-center p-4">
      <div className="max-w-md w-full space-y-8 bg-slate-800/40 backdrop-blur-xl p-12 rounded-5xl border border-slate-700/50 shadow-2xl">
        ...
        {!submitted ? (
          <>
            <p className="text-slate-400 text-sm font-medium text-center leading-6">
              Enter the email address associated with your admin account. We will send you a
              password reset link.
            </p>
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              <div className="space-y-2">
                ...
                <input
                  ...
                  className="w-full px-6 py-4 bg-slate-700/30 border border-slate-600/50 rounded-2xl focus:ring-2 focus:ring-violet-600 focus:border-transparent transition-all font-bold text-white placeholder-slate-500 disabled:opacity-60"
          ...
          <div className="text-center space-y-4">
            <p className="text-slate-400 font-medium leading-6">
              Check your email for password reset instructions. You will be redirected shortly.
            </p>
          </div>
        )}
